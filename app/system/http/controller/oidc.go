...
func (c OIDC) Login(ctx *gin.Context) {
	...
	state, err := function.MakeOIDCState(nonce, oidcStateSecret(), 10*time.Minute)
	if err != nil {
		c.JsonResponseWithServerError(ctx, err)
		return
	}
...
func (c OIDC) Callback(ctx *gin.Context) {
	...
	nonce, err := function.VerifyOIDCState(ctx.Query("state"), oidcStateSecret())
	if err != nil {
		c.JsonResponseWithError(ctx, err, http.StatusBadRequest)
		return
	}

	info, err := oidcLogic.ExchangeCode(ctx.Request.Context(), cfg, cfg.RedirectURI, code, nonce)
	if err != nil {
		c.JsonResponseWithServerError(ctx, err)
		return
	}
