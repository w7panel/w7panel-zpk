...
{{- define "plugin.traditionStorageClaimName" -}}
{{- required "应用插件必须指定传统应用 PVC_NAME" .Values.PVC_NAME -}}
{{- end -}}

{{/* Return the selected environment domain as a filesystem-safe directory name. */}}
{{- define "plugin.codeInstallDirectory" -}}
{{- $domain := required "应用插件必须指定传统应用 DOMAIN_URL" .Values.DOMAIN_URL -}}
{{- $domain = regexReplaceAll "^https?://" $domain "" -}}
{{- $domain = trimSuffix "/" $domain -}}
{{- if not (regexMatch "^[A-Za-z0-9._-]+$" $domain) -}}
{{- fail (printf "传统应用域名无效: %s" $domain) -}}
{{- end -}}
{{- $domain -}}
{{- end -}}
